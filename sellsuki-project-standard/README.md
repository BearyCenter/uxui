...
| Stack | Sellsuki DS 1.0 / DS 2.0 mandatory | Client's stack หรือ modern framework choice (ไม่ใช้ DS) |
...
└── references/                      ← Decision support
    ...
    ├── tech-stack-guide.md          Client stack vs modern framework decision tree
...
Project ใช้ **stack ของลูกค้า** (ถ้ามี codebase อยู่แล้ว) หรือ **modern framework choice** (greenfield, default Next.js + TS + Tailwind + shadcn/ui). **ไม่ใช้ Sellsuki DS 1.0 / DS 2.0** — DS เป็นของ Sellsuki internal product เท่านั้น (ใช้ใน `sellsuki-product-standard` skill)

ดู `references/tech-stack-guide.md` สำหรับ decision tree + default stack recommendations
...
- **Stack ต่างกันโดยสิ้นเชิง:** product skill ใช้ Sellsuki DS 1.0/2.0 mandatory; project skill ใช้ client's stack หรือ modern framework choice (ห้ามใช้ DS)
